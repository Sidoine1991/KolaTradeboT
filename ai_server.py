...
# Importer le suivi des performances des indicateurs

try:

    from indicator_performance import PerformanceTracker, detect_market_uncertainty
    ...
    print("Avertissement: Le module indicator_performance n'est pas disponible. La pondération dynamique des indicateurs est désactivée.")
...
# DURÉES DE CACHE OPTIMISÉES POUR RÉACTIVITÉ

CACHE_DURATION = 30  # 30 secondes (par défaut)

CACHE_DURATION_SHORT = 15  # 15 secondes pour les décisions fréquentes (M1/M5, marchés volatils)

CACHE_DURATION_VERY_SHORT = 3  # 3 secondes pour les mouvements détectés (scalping)



# CONFIGURATION POUR L'ANALYSE PARALLÈLE

MAX_PARALLEL_REQUESTS = 3  # Limite les requêtes concurrentes vers MT5

ANALYSIS_TIMEOUT = 15.0    # Timeout en secondes pour chaque analyse
...
def apply_confidence_thresholds(
    ...
    Applique les seuils de confiance et la pondération dynamique à une décision.

    

    Args:

        action: Action proposée (buy/sell/hold)

        confidence: Niveau de confiance initial (0-1)

        reason: Raison de la décision

        symbol: Symbole tradé (optionnel, utilisé pour la personnalisation)

        df: Données de marché (optionnel, utilisé pour l'analyse de volatilité)

        indicator_values: Dictionnaire des valeurs d'indicateurs actuelles

        

    Returns:

        Tuple: (action_ajustée, confiance_ajustée, raison_mise_à_jour)

    """

    # Constantes de configuration

    MIN_CONFIDENCE = 0.60  # 60% de confiance minimum pour considérer un signal

    FORCE_HOLD_THRESHOLD = 0.68  # Seuil pour forcer HOLD si confiance < 68%

    UNCERTAINTY_THRESHOLD = 0.7  # Au-delà de ce seuil, on réduit la confiance

    

    # 1. Vérifier les seuils de confiance de base

    if confidence < MIN_CONFIDENCE:

        return "hold", max(0.1, confidence), f"Confiance trop faible ({confidence:.1%} < {MIN_CONFIDENCE:.0%})"

    

    # 2. Calculer l'incertitude du marché si les données sont disponibles

    market_uncertainty = 0.5  # Valeur par défaut

    

    # Utiliser le module indicator_performance si disponible

    if PERFORMANCE_TRACKING_AVAILABLE and df is not None and not df.empty and indicator_values:

        try:

            # Calculer l'incertitude du marché avec le module dédié

            market_uncertainty = detect_market_uncertainty(

                symbol=symbol,

                indicators=indicator_values,

                timeframe="M1"  # ou récupérer le timeframe depuis df si disponible

            )

            

            # Ajuster la confiance en fonction de l'incertitude

            if market_uncertainty > UNCERTAINTY_THRESHOLD:

                # Réduire la confiance proportionnellement à l'incertitude

                reduction = (market_uncertainty - UNCERTAINTY_THRESHOLD) / (1 - UNCERTAINTY_THRESHOLD)

                confidence = confidence * (1 - reduction * 0.5)  # Jusqu'à 50% de réduction

                reason += f" | Ajustement incertitude marché ({market_uncertainty:.1%})"

                

        except Exception as e:

            logger.warning(f"Erreur lors du calcul d'incertitude du marché: {e}")

            # En cas d'erreur, utiliser la méthode de secours

            market_uncertainty = calculate_uncertainty_score(df, symbol) if df is not None else 0.5

    

    # 3. Appliquer la pondération dynamique si le symbole est fourni

    if symbol and PERFORMANCE_TRACKING_AVAILABLE:

        try:

            # Initialiser le suivi des performances pour ce symbole

            tracker = PerformanceTracker(symbol=symbol)

            

            # Obtenir les poids dynamiques des indicateurs

            indicator_weights = tracker.get_all_weights()

            

            if indicator_weights:

                # Calculer un facteur de pondération global basé sur les poids des indicateurs
                ...
                # Si nous avons des valeurs d'indicateurs actuelles, les utiliser pour la pondération
                ...
                if indicator_values:
                    ...
                    for indicator, value in indicator_values.items():
                        ...
                        if indicator in indicator_weights:
                            ...
                            # Normaliser la valeur de l'indicateur entre 0 et 1 si nécessaire

                            # (cette partie dépend de la plage de valeurs de chaque indicateur)

                            norm_value = min(1.0, max(0.0, value / 100.0 if indicator == 'rsi' else value))

                            weighted_confidence += norm_value * weight

                            total_weight += weight

                

                # Si nous avons pu calculer une confiance pondérée, l'utiliser

                if total_weight > 0:

                    weighted_confidence = weighted_confidence / total_weight

                    # Combiner avec la confiance initiale (moyenne pondérée)

                    confidence = (confidence * 0.7) + (weighted_confidence * 0.3)

                    reason += f" | Confiance pondérée: {weighted_confidence:.1%}"

                    

        except Exception as e:

            logger.warning(f"Erreur lors de l'application de la pondération dynamique: {e}")

    

    # 4. Appliquer les seuils finaux

    if confidence < FORCE_HOLD_THRESHOLD:

        # Si la confiance est trop faible, forcer HOLD

        return "hold", confidence, f"Confiance insuffisante après ajustements ({confidence:.1%} < {FORCE_HOLD_THRESHOLD:.0%})"

    

    # 5. Journaliser la décision finale

    logger.info(

        f"🎯 Décision finale: {action.upper()} avec confiance {confidence:.1%} "

        f"(incertitude: {market_uncertainty:.1%}) - {reason}"

    )

    

    # Retourner la décision avec la confiance ajustée

    return action, min(0.95, confidence), reason
...
def get_cached_decision(symbol: str) -> Optional[Dict]:
    ...
    Utilise des durées de cache variables en fonction de la volatilité détectée.
    ...
    if symbol in decision_cache:
        ...
        # Déterminer la durée de cache appropriée

        cache_age = current_time - cache_timestamps.get(symbol, 0)

        

        # Par défaut, utiliser CACHE_DURATION_SHORT pour une réactivité accrue

        cache_expiry = CACHE_DURATION_SHORT

        

        # Si le symbole est très volatile (comme Boom/Crash), utiliser une durée plus courte

        if is_boom_crash_symbol(symbol):

            cache_expiry = CACHE_DURATION_VERY_SHORT

            

        if cache_age < cache_expiry:

            logger.debug(f"✅ Cache trouvé pour {symbol} (âge: {cache_age:.1f}s, expiry: {cache_expiry}s)")
...
def cache_decision(symbol: str, decision_data: Dict, duration: Optional[int] = None):

    """

    Stocke une décision dans le cache avec une durée personnalisée.
    ...
    Args:
        ...
        decision_data: Données de décision à mettre en cache

        duration: Durée de cache en secondes (si None, utilise la durée par défaut)
    ...
    cache_duration = duration if duration is not None else CACHE_DURATION_SHORT

    logger.debug(f"💾 Décision mise en cache pour {symbol} (durée: {cache_duration}s)")
...
def calculate_uncertainty_score(df: pd.DataFrame, symbol: str) -> float:

    """

    Calcule un score d'incertitude du marché (0-1) basé sur plusieurs facteurs.
    ...
    Returns:
        ...
        Score d'incertitude entre 0 (certain) et 1 (très incertain)
    ...
    try:
        ...
        # 1. Volatilité (écart-type des rendements)
        ...
        if 'close' in df.columns:
            ...
            if not returns.empty:
                ...
                # Normaliser la volatilité (à adapter selon le symbole)

                norm_volatility = min(1.0, volatility / 0.02)  # 2% de volatilité = max

                uncertainty_score += 0.4 * norm_volatility

                factors.append(f"volatilité={norm_volatility:.2f}")
        ...
        if 'adx' in df.columns and not df['adx'].isna().all():
            ...
            if adx < 20:
                ...
                uncertainty_score += 0.3 * (1 - adx / 20)

                factors.append(f"adx_bas={adx:.1f}")

        

        # 3. Volume (écart par rapport à la moyenne)
        ...
        if 'volume' in df.columns and not df['volume'].isna().all():
            ...
            if volume_mean > 0:
                ...
                # Volume très bas ou très élevé = plus d'incertitude

                if volume_ratio < 0.5 or volume_ratio > 2.0:

                    vol_factor = abs(1 - volume_ratio) / 2

                    uncertainty_score += 0.2 * min(1.0, vol_factor)

                    factors.append(f"volume_anormal={volume_ratio:.2f}")

        

        # 4. Écart par rapport aux moyennes mobiles

        if all(col in df.columns for col in ['close', 'sma50', 'sma200']):
            ...
            # Distance relative aux MMs

            dist_sma50 = abs(price - sma50) / sma50 if sma50 > 0 else 0

            dist_sma200 = abs(price - sma200) / sma200 if sma200 > 0 else 0

            

            # Si le prix est loin des MMs, plus d'incertitude

            if dist_sma50 > 0.02 or dist_sma200 > 0.05:

                uncertainty_score += 0.1 * min(1.0, (dist_sma50 * 20 + dist_sma200 * 10) / 2)

                factors.append(f"ecart_mm={max(dist_sma50, dist_sma200):.2%}")
        ...
        # Journaliser les facteurs d'incertitude si le score est élevé

        if uncertainty_score > 0.5:

            logger.info(

                f"⚠️ Incertitude élevée pour {symbol}: {uncertainty_score:.1%} "
        ...
        logger.warning(f"Erreur calcul score d'incertitude pour {symbol}: {e}")

        return 0.5  # En cas d'erreur, retourner une incertitude moyenne
...
def detect_spike_pattern(df: pd.DataFrame, symbol: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "stop_loss": round(float(last_candle['close'] - (last_candle['close'] * 0.005)), 2) if pd.notna(last_candle['close']) else None,

        "take_profit": round(float(last_candle['close'] + (last_candle['close'] * 0.01)), 2) if pd.notna(last_candle['close']) else None,

        "position_size": 0.01,  # Taille fixe pour les spikes

        "reasoning": [

            f"Spike {spike_direction} détecté",

            f"Changement prix: {spike_criteria['price_spike']:.2f}%",

            f"Volume ratio: {spike_criteria['volume_spike']:.1f}x",

            f"Confiance: {spike_confidence:.0f}%"
        ...
        "spike_info": {
            ...
            for k, v in spike_criteria.items()
...
def calculate_atr(df: pd.DataFrame, period: int = 14) -> pd.Series:
    ...
    if not all(col in df.columns for col in ['high', 'low', 'close']):
        ...
        raise ValueError("Le DataFrame doit contenir les colonnes 'high', 'low', 'close'")
...
def get_multi_model_ml_decision(symbol: str, df_ohlc: pd.DataFrame) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Ajouter la colonne timestamp si manquante
        if 'timestamp' not in df_ohlc.columns:
            if 'time' in df_ohlc.columns:
                df_ohlc['timestamp'] = pd.to_datetime(df_ohlc['time'])
            else:
                df_ohlc['timestamp'] = pd.date_range(start='2024-01-01', periods=len(df_ohlc), freq='1min')
...
f"Base={base_confidence:.1%} | H4/D1={long_term_bonus:.1%} | H1+H4/D1={long_term_alignment_bonus:.1%}% | "
f"M5+H1={medium_term_bonus:.1%}% | Align={alignment_bonus:.1%}% | FINAL={confidence:.1%}%")
...
async def analyze_timeframe(symbol: str, timeframe: str, count: int = 400) -> Dict[str, Any]:

    """Analyse une timeframe spécifique avec gestion des erreurs et timeout"""

    start_time = time.time()

    logger.debug(f"Starting {timeframe} analysis for {symbol}")
    ...
    try:
        ...
        df = await asyncio.wait_for(
            ...
            asyncio.get_event_loop().run_in_executor(
                ...
                lambda: get_historical_data_mt5(symbol, timeframe, count)
        ...
        if df is not None and not df.empty:
            ...
            # Détection des tendances avec gestion des erreurs

            try:

                trendlines = await asyncio.get_event_loop().run_in_executor(

                    None,

                    lambda: detect_trendlines(df)

                )

                duration = time.time() - start_time

                logger.info(f"✅ {timeframe} analysis completed in {duration:.2f}s ({len(df)} candles)")
                ...
                logger.error(f"❌ Error in trendline detection for {symbol} {timeframe}: {str(e)}")
        ...
        logger.error(f"⏱️ {timeframe} analysis timed out after {ANALYSIS_TIMEOUT}s")
        ...
        logger.error(f"❌ Error in {timeframe} analysis: {str(e)}", exc_info=True)
...
async def get_technical_analysis(symbol: str) -> AnalysisResponse:
    ...
    """Effectue une analyse technique complète avec parallélisation"""

    logger.info(f"🚀 Starting parallel technical analysis for {symbol}")
