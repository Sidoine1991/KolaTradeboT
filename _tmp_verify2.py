...
df0['session'] = (df0.time.dt.hour.values >= 0) & (df0.time.dt.hour.values < 24)
...
pv=0.1; fvg_bull=(bull>=3.0*pv)&(bull>0); fvg_bear=(bear>=3.0*pv)&(bear>0)
...
t_arr=df.time.values; px_arr=df.close.values; atr_v=df.atr.values; day_arr=np.array([str(t)[:10] for t in t_arr])
htf=df.htf.values; rsi_v=df.rsi.values

print('Testing: FVG(3pips)+BOS(10)+HTF=-1 + RSI[22,78] (original working params)')
sig=np.zeros(len(df),dtype=np.int8); ses=np.ones(len(df),dtype=bool)
sig[(fvg_bear[:len(df)])&ses]=-1
sig[(bos_bear[:len(df)])&(~np.isin(np.where(fvg_bear[:len(df)],-1,0),[-1]))&ses]=-1
sig[~((rsi_v>22)&(rsi_v<78))]=0
sig[htf!=-1]=0
...
print(f'  sig count={np.count_nonzero(sig)}')
...
ret=(equity-CAP)/CAP
print(f'  RESULT: trades={trades} WR={wr:.1f}% PF={pf:.2f} Return={ret*100:+.1f}% DD={mdd:.1f}%')
