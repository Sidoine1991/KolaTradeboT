#ifndef SMC_ENHANCED_OTE_CAPITAL_MANAGEMENT_MQH
#define SMC_ENHANCED_OTE_CAPITAL_MANAGEMENT_MQH

// -----------------------------------------------------------------------------
// Stub (missing include) for OTE "coupled confirmations" M5+H1.
//
// Your SMC_Universal.mq5 references:
// - OTE_RequireM5H1CoupledConfirmation
// - OTE_M5H1MinConfluenceScore
// - CheckM5H1CoupledConfirmation(symbol, dir, price)
//
// This file restores compilation and provides a meaningful scoring:
// - EMA fast/slow alignment on M5 + H1 in the requested direction
// - price proximity to the slow EMA (normalized using ATR M5)
// Returns a score in [0..100] used by OTE filters.
// -----------------------------------------------------------------------------

input bool   OTE_RequireM5H1CoupledConfirmation = true;
input double OTE_M5H1MinConfluenceScore = 65.0; // percent

double CheckM5H1CoupledConfirmation(const string symbol, const string dirRaw, const double price)
{
   string dir = dirRaw;
   StringToUpper(dir);

   // Handles (simple EMA set; cheap & stable)
   int fastM5 = iMA(symbol, PERIOD_M5, 9, 0, MODE_EMA, PRICE_CLOSE);
   int slowM5 = iMA(symbol, PERIOD_M5, 21, 0, MODE_EMA, PRICE_CLOSE);
   int fastH1 = iMA(symbol, PERIOD_H1, 9, 0, MODE_EMA, PRICE_CLOSE);
   int slowH1 = iMA(symbol, PERIOD_H1, 21, 0, MODE_EMA, PRICE_CLOSE);
   int atrM5  = iATR(symbol, PERIOD_M5, 14);

   if(fastM5 == INVALID_HANDLE || slowM5 == INVALID_HANDLE ||
      fastH1 == INVALID_HANDLE || slowH1 == INVALID_HANDLE || atrM5 == INVALID_HANDLE)
   {
      if(fastM5 != INVALID_HANDLE) IndicatorRelease(fastM5);
      if(slowM5 != INVALID_HANDLE) IndicatorRelease(slowM5);
      if(fastH1 != INVALID_HANDLE) IndicatorRelease(fastH1);
      if(slowH1 != INVALID_HANDLE) IndicatorRelease(slowH1);
      if(atrM5  != INVALID_HANDLE) IndicatorRelease(atrM5);
      return 0.0;
   }

   double fM5[], sM5[], fH1[], sH1[], atrArr[];
   ArraySetAsSeries(fM5, true);
   ArraySetAsSeries(sM5, true);
   ArraySetAsSeries(fH1, true);
   ArraySetAsSeries(sH1, true);
   ArraySetAsSeries(atrArr, true);
   ArrayResize(fM5, 1); ArrayResize(sM5, 1);
   ArrayResize(fH1, 1); ArrayResize(sH1, 1);
   ArrayResize(atrArr, 1);

   if(CopyBuffer(fastM5, 0, 0, 1, fM5) < 1 ||
      CopyBuffer(slowM5, 0, 0, 1, sM5) < 1 ||
      CopyBuffer(fastH1, 0, 0, 1, fH1) < 1 ||
      CopyBuffer(slowH1, 0, 0, 1, sH1) < 1 ||
      CopyBuffer(atrM5,  0, 0, 1, atrArr) < 1)
   {
      IndicatorRelease(fastM5);
      IndicatorRelease(slowM5);
      IndicatorRelease(fastH1);
      IndicatorRelease(slowH1);
      IndicatorRelease(atrM5);
      return 0.0;
   }

   double fastM5v = fM5[0];
   double slowM5v = sM5[0];
   double fastH1v = fH1[0];
   double slowH1v = sH1[0];
   double atr     = atrArr[0];
   if(atr <= 0.0)
      atr = MathMax(1e-9, price * 0.001);

   bool m5Aligned = (dir == "BUY") ? (fastM5v > slowM5v) : (fastM5v < slowM5v);
   bool h1Aligned = (dir == "BUY") ? (fastH1v > slowH1v) : (fastH1v < slowH1v);

   // Proximity: closer to slow EMA => higher score
   double dist = MathAbs(price - slowM5v);
   double proximity01 = 1.0 - MathMin(1.0, dist / (atr * 1.5)); // 0..1

   double score = 0.0;
   score += m5Aligned ? 55.0 : 10.0;
   score += h1Aligned ? 35.0 : 10.0;

   // Adjust with proximity
   score *= (0.55 + 0.45 * proximity01);
...
#endif // SMC_ENHANCED_OTE_CAPITAL_MANAGEMENT_MQH
...
OTEConfirmationScore EvaluateOTEConfirmations(string symbol, string direction, double entryPrice, double fibLevel)
{
   ...
   // 7. Setup récent
   bool freshSetup = true;  // À implémenter avec age réel
   if(freshSetup)
   {
      score.totalConfirmations++;
   ...
   // 8. Propreté zone OTE
   bool zoneCleanliness = true;  // Simplification - à implémenter avec logique réelle
   if(zoneCleanliness)
   {
      score.totalConfirmations++;
      score.qualityScore += 10.0;
...
bool CheckVolumeConfirmation(string symbol)
{
   ...
   for(int i = 0; i < ArraySize(avgVolume); i++)
      volumeSum += (double)avgVolume[i];

   double avgVol = volumeSum / (double)ArraySize(avgVolume);
...
double CheckMomentumOnTimeframe(string symbol, string direction, ENUM_TIMEFRAMES tf)
{
   ...
   if(macdHandle != INVALID_HANDLE)
   {
      ...
      if(CopyBuffer(macdHandle, 0, 0, 1, macdBuf) >= 1) macdMain = macdBuf[0];
      if(CopyBuffer(macdHandle, 1, 0, 1, signalBuf) >= 1) macdSignal = signalBuf[0];
   }
   ...
   {
      double stochBuf[], signalBuf[];
      ArraySetAsSeries(stochBuf, true);
      ArraySetAsSeries(signalBuf, true);
      
      if(CopyBuffer(stochHandle, 0, 0, 1, stochBuf) >= 1) stochMain = stochBuf[0];
      if(CopyBuffer(stochHandle, 1, 0, 1, signalBuf) >= 1) stochSignal = signalBuf[0];
   }
