...
CAP = 10000.0; RISK = 0.3; SPD = 0.1; SL_M = 2.5; TP_M = 3.0
ATR_P = 14; EMA_S = 50; BOS_N = 10; SPIKE_R = 1.5; POST_SPK_B = 20
...
df0['es'] = df0.close.ewm(span=EMA_S, adjust=False).mean().values

c0=df0.close.values; h0=df0.high.values; lw0=df0.low.values
bull=np.empty_like(c0); bear=np.empty_like(c0)
bull[:-1]=lw0[1:]-h0[:-1]; bull[-1]=0; bear[:-1]=lw0[:-1]-h0[1:]; bear[-1]=0
pv = 0.1
...
results = []
for rsi_l in [20,25,30,35,40]:
    for rsi_h in [65,70,75,80,85]:
        sig = sig0.copy()
        ...
        for i in range(need, len(df)):
            ...
            if (not in_pos) and (i-entry_bar)>=1 and sig[i]==-1:
                ...
                if sl_d>0:
                    ...
                    pos_ebar=i; pos_etime=ts; pos_entry=px
                    pos_sl=px+sl_d; pos_tp=px-tp_d; pos_sz=sz; in_pos=True; entry_bar=i
        if trades>0:
            wr=wins/trades*100
            pf=gross_p/abs(gross_l) if gross_l!=0 else 999
            ret=(equity-CAP)/CAP
            flag='OK' if (wr>=55 and trades>=200 and pf>=1.1) else ''
            results.append((rsi_l,rsi_h,np.count_nonzero(sig),trades,wr,pf,ret*100,mdd,flag))

out = r'D:\Dev\TradBOT\_grid_scan.csv'
import csv
with open(out,'w',newline='') as f:
    w=csv.writer(f); w.writerow(['RSI_L','RSI_H','sig_count','trades','wr','pf','ret','mdd','flag'])
    for row in results: w.writerow(row)
print(f'Done {len(results)} combos -> {out}', flush=True)
