    ...
    if not hasattr(app.state, "db_pool"):
        ...
        try:
            ...
            if "render.com" in DATABASE_URL.lower() and "sslmode" not in DATABASE_URL.lower():
                # Ajouter sslmode=require pour Supabase PostgreSQL
                separator = "?" if "?" not in dsn else "&"
                dsn = f"{dsn}{separator}sslmode=require"
                logger.info("📝 Ajout de sslmode=require pour Supabase PostgreSQL")
...
if RUNNING_ON_RENDER:
    ...
    logger.info(f"Mode Supabase activé - Utilisation des dossiers temporaires:")
