...
def _get_supabase_config(strict: bool = True) -> Tuple[str, str]:
    """
    AWS RDS Mode - Supabase disabled.
    Returns empty strings (no error).
    """
    return "", ""
...
async def _compute_propice_top_from_trade_feedback(
    ...
    if r.status_code not in (200, 206):
        logger.debug("Supabase (disabled):  trade_feedback HTTP {r.status_code}: {r.text[:200]}")
...
async def get_symbol_propice_status(symbol: str, timeframe: str = "M1", lookback_days: int = 14):
    ...
    try:
        ...
        if r.status_code not in (200, 206):
            logger.debug("Supabase (disabled):  HTTP {r.status_code}: {r.text[:200]}")
        rows = r.json() if r.text else []
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        if rows_day:
            ...
            if r1.status_code not in (200, 201, 204):
                logger.debug("Supabase (disabled):  symbol_trade_stats upsert day HTTP {r1.status_code}: {r1.text[:200]}")
        if rows_month:
            r2 = await client.post(f"{supabase_url}/rest/v1/symbol_trade_stats", headers=up_headers, json=rows_month)
            if r2.status_code not in (200, 201, 204):
                logger.debug("Supabase (disabled):  symbol_trade_stats upsert month HTTP {r2.status_code}: {r2.text[:200]}")
...
def _call_ollama_local(prompt: str, model: str = "qwen3.5:4b", timeout: int = 10) -> Optional[str]:
    """Appelle le modèle Ollama local optimisé pour trading rapide (< 10s)"""
    try:
...
async def decision(request: DecisionRequest):
    """
    Endpoint /decision - Redirection vers decision360 pour compatibilité EA.
    Utilisé par SMC_Universal.mq5 pour obtenir décisions de trading.
    """
    return await decision_360(Decision360Request(
        symbol=request.symbol,
        bid=request.bid,
        ask=request.ask,
        rsi=request.rsi,
        ...
        ema_fast_m1=request.ema_fast_m1,
        ema_slow_m1=request.ema_slow_m1,
        ema_fast_m5=getattr(request, 'ema_fast_m5', 0.0),
        ema_slow_m5=getattr(request, 'ema_slow_m5', 0.0),
        ema_fast_h1=getattr(request, 'ema_fast_h1', 0.0),
        ema_slow_h1=getattr(request, 'ema_slow_h1', 0.0),
        dir_rule=getattr(request, 'dir_rule', 0),
        timeframe=request.timeframe,
        volatility_compression=getattr(request, 'volatility_compression', 1.0),
        price_acceleration=getattr(request, 'price_acceleration', 0.0),
        volume_spike=getattr(request, 'volume_spike', False),
        spike_probability=getattr(request, 'spike_probability', 0.0),
        timestamp=getattr(request, 'timestamp', datetime.now(timezone.utc).isoformat()),
    ))
