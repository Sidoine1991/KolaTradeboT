...
class MT5AIClient:
    ...
    def check_neutral_decision_closure_optimized(self, symbol, decision):
        ...
        try:
            ...
            return False
    def monitor_all_symbols_neutral_decisions(self):
        """Surveille tous les symboles pour décisions neutres et ferme les positions (OPTIMISÉ)"""
        try:
            # Obtenir tous les symboles avec des positions actives
            positions = mt5.positions_get()
            if not positions:
                return
            
            # Créer un set des symboles actifs
            active_symbols = set(position.symbol for position in positions)
            
            # Pour chaque symbole actif, vérifier la décision actuelle
            for symbol in active_symbols:
                try:
                    # Récupérer le dernier signal pour ce symbole
                    signal_data = self.get_latest_signal(symbol)
                    if signal_data:
                        decision = signal_data.get('decision', '')
                        # Vérifier si c'est une décision neutre (version optimisée)
                        if self.check_neutral_decision_closure_optimized(symbol, decision):
                            self.add_symbol_to_blacklist(symbol, "DÉCISION NEUTRE")
                    else:
                        # Si pas de signal disponible, considérer comme neutre
                        if self.check_neutral_decision_closure_optimized(symbol, "NO_SIGNAL"):
                            self.add_symbol_to_blacklist(symbol, "AUCUN SIGNAL")
                            
                except Exception as e:
                    logger.error(f"Erreur surveillance symbole {symbol}: {e}")
                    
        except Exception as e:
            logger.error(f"Erreur surveillance décisions neutres: {e}")
    ...
    def check_neutral_decision_closure(self, symbol, decision):
        ...
        try:
            ...
            if positions_closed > 0:
                ...
                if symbol in self.ema_scalping_strategy.active_scalping_strategies:
    ...
    def monitor_all_symbols_neutral_decisions(self):
        """Surveille tous les symboles pour décisions neutres et ferme les positions"""
        try:
            # Obtenir tous les symboles avec des positions actives
            positions = mt5.positions_get()
            if not positions:
            ...
            for symbol in active_symbols:
                try:
                    ...
                    if signal_data:
                        ...
                        # DEBUG: Loguer la décision pour chaque symbole actif
                        logger.info(f"🔍 Surveillance {symbol}: Décision='{decision}' | Signal='{signal}' | Confiance={confidence}%")
                        
                        # Vérifier si c'est une décision neutre
                        if self.check_neutral_decision_closure(symbol, decision):
                            # Ajouter le symbole à la liste noire temporaire
                            self.add_symbol_to_blacklist(symbol, "DÉCISION NEUTRE")
                    else:
                        # Si pas de signal disponible, considérer comme neutre par sécurité
                        logger.warning(f"⚠️ Aucun signal disponible pour {symbol} - Considéré comme NEUTRE")
                        if self.check_neutral_decision_closure(symbol, "NO_SIGNAL"):
                            self.add_symbol_to_blacklist(symbol, "AUCUN SIGNAL")
    ...
    def get_latest_signal(self, symbol):
        ...
        try:
            # Utiliser l'API locale pour récupérer le dernier signal
            import requests
            
            url = f"{LOCAL_API_URL}/get_signal"
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"⚠️ Erreur API signal {symbol}: Status {response.status_code}")
                return None
                
        except Exception as e:
            logger.warning(f"⚠️ Erreur récupération signal {symbol}: {e}")
            return None
    ...
    def run(self):
        ...
        logger.info("🚀 Demarrage du client MT5 AI Ultra-Rapide")
