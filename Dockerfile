...
# Commande de démarrage — lancer ai_server_clean.py directement (gère ses propres arguments --host et --port)
# Render injecte PORT ; défaut 10000 pour build local.
ENV PORT=10000
CMD ["sh", "-c", "exec python Python/ai_server_clean.py --host 0.0.0.0 --port ${PORT:-10000}"]
