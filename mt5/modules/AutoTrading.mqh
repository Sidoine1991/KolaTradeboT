...
//| Simplifi� pour MQL5 (OrderModify natif, pas de CTrade)            |
//+------------------------------------------------------------------+
...
input bool   UseAutoReentry         = true;     // Re-entry sur m�me signal
...
bool AutoPlaceOrder(string symbol, string direction, double entry, double sl, double tp, double lot)
{
   ...
   request.comment = "AUTO-TRADE";
    ...
    if(stopLevel > 0)
    {
       ...
       if(MathAbs(entry - request.sl) < minDist)
       {
          request.sl = (request.type == ORDER_TYPE_BUY) ?
                       NormalizeDouble(entry - minDist, digits) :
                       NormalizeDouble(entry + minDist, digits);
          Print("AutoTrade: SL ajusté au stopLevel → ", DoubleToString(request.sl, digits));
       }
       if(MathAbs(entry - request.tp) < minDist)
       {
          request.tp = (request.type == ORDER_TYPE_BUY) ?
                       NormalizeDouble(entry + minDist, digits) :
                       NormalizeDouble(entry - minDist, digits);
          Print("AutoTrade: TP ajusté au stopLevel → ", DoubleToString(request.tp, digits));
       }
    }
