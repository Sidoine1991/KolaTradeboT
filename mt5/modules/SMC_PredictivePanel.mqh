...
void SMCPP_DrawPath(const double &prices[])
{
   ...
   for(int i = 0; i < n - 1; i++)
   {
      ...
      ObjectSetInteger(0, seg, OBJPROP_COLOR, clrWhite);
...
void SMCPP_DrawPanel(const SMCPP_PanelState &st)
{
   ...
   color obClr = (st.direction == "BUY") ? clrDodgerBlue : clrOrange;
   ...
   if(top > bot && st.price > 0)
      SMCPP_DrawZoneRect("PRED_PLAY_ZONE", top, bot, clrRoyalBlue);

   SMCPP_DrawHLine("PRED_LVL_LOW", st.levelLow, clrGold, "Low H1 - M15");
   SMCPP_DrawHLine("PRED_LVL_HIGH", st.levelHigh, clrGold, "High récent");
   SMCPP_DrawHLine("PRED_TP1", st.tp1, clrLime, "TP1", STYLE_SOLID);
   SMCPP_DrawHLine("PRED_TP2", st.tp2, clrGreen, "TP2", STYLE_SOLID);
   SMCPP_DrawHLine("PRED_SL", st.sl, clrRed, "SL", STYLE_SOLID);
   ...
   ObjectSetInteger(0, "PRED_LBL_STATUS", OBJPROP_COLOR,
                   st.alert == "ACTIVE" ? clrLime : st.alert == "FORMING" ? clrOrange : clrSilver);
