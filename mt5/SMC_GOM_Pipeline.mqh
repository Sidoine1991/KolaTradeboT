...
// Stocker le verdict pour un symbole donné
...
// Lire le verdict en cache pour un symbole (retourne 0 si pas trouvé ou stale > 120s)
int SMCGP_GetCachedVerdictNum(const string symbol)
{
   ...
   for(int i = 0; i < g_gomCacheCount; i++)
   {
      if(g_gomCacheSym[i] == sym)
      {
         // Stale si > 120 secondes
         if(TimeCurrent() - g_gomCacheTime[i] > 120) return -999;
         return g_gomCacheVn[i];
      }
   }
   return -999; // pas de cache = inconnu
}
datetime g_smcLastPipelineFail= 0;
string   g_smcFailedPipelineId= "";
int      g_smcPipelineFailCount = 0;
// ── Stabilité GOM (anti-whipsaw) ────────────────────────────────────
datetime g_smcGomStableSince = 0;    // timestamp quand le verdict est devenu stable
int      g_smcGomStableVn    = 0;    // verdict stable (même signe + |vn|>=2)
                                  ...
                                  const int prevVnum, const string &prevVerdict)
{
   ...
   // ── Mise à jour stabilité (anti-whipsaw) ─────────────────────────
   // Si le verdict change de direction OU |vn| < 2, on reset le timer
   // Si le verdict reste dans la même direction avec |vn| >= 2, on maintient
   if(newVnum == 0 || MathAbs(newVnum) < 2)
   {
      // Verdict faible ou zéro : pas stable
      g_smcGomStableSince = 0;
      g_smcGomStableVn = 0;
   }
   else if(g_smcGomStableSince == 0 || 
           (newVnum > 0 && g_smcGomStableVn <= 0) ||
           (newVnum < 0 && g_smcGomStableVn >= 0))
   {
      // Changement de direction : on reset le timer
      g_smcGomStableSince = TimeCurrent();
      g_smcGomStableVn = newVnum;
   }
   // Sinon : même direction, on garde le timestamp original (pas de reset)
}
...
bool SMCGP_GOMAllowsDirectionEx(const int dir, const bool requireOBTouch)
{
   ...
   // ── Vérification stabilité anti-whipsaw ──────────────────────────
   // Le verdict doit être stable (même direction, |vn|>=2) depuis au moins GOMEntryStabilitySec secondes
   if(GOMEntryStabilitySec > 0 && g_smcGomStableSince > 0)
   {
      int stableDuration = (int)(TimeCurrent() - g_smcGomStableSince);
      if(stableDuration < GOMEntryStabilitySec)
      {
         Print("[GOM-ALLOW] Rejeté: NOT_STABLE_YET ", stableDuration, "s < ", GOMEntryStabilitySec, "s | vn=", g_smcGomVerdictNum);
         return false;
      }
   }
                              ...
                              double &sl, double &tp)
{
   ...
   StringReplace(sc, " ", "");
   bool isWeltrade = SMC_IsWeltradeSymbol(sym);
   bool isDerivBoomCrash = (StringFind(symU, "BOOM") >= 0 || StringFind(symU, "CRASH") >= 0);
   bool isWeltradeSynth = (StringFind(symU, "PAINX") >= 0 || StringFind(symU, "GAINX") >= 0 ||
                           StringFind(symU, "TRENDX") >= 0);

   // Pour Weltrade synthetics (PainX/GainX), distances adaptées (pas 8%/10% comme Boom/Crash Deriv)
   if(isWeltradeSynth && px > 0)
   {
      slDist = MathMax(slDist, px * 0.003);  // 0.3% minimum SL (~270 pts for painx 400)
      tpDist = MathMax(tpDist, px * 0.005);  // 0.5% minimum TP (~450 pts for painx 400)
   }
   // Pour Deriv Boom/Crash seulement, distances beaucoup plus grandes
   else if(isDerivBoomCrash && px > 0)
   {
...
void SMCGP_PollAndExecutePipeline()
{
   ...
   if(UseGOMVerdictFilter && g_smcGomConnected && g_smcGomVerdictNum == 0)
   {
    ...
    // Gate direction GOM — TOUS les ordres (pipeline inclus)
    // Indépendant de g_smcGomConnected : utilise g_smcGomVerdictNum même si poller down
    if(UseGOMVerdictFilter && g_smcGomVerdictNum != 0)
    {
       int gomSign = (g_smcGomVerdictNum > 0) ? 1 : -1;
       int dirSign = (action == "BUY") ? 1 : -1;
       if(gomSign != dirSign && MathAbs(g_smcGomVerdictNum) >= 2)
       {
          string effVerdict = (MathAbs(g_smcGomVerdictNum) >= 3) ? "PERFECT" : "GOOD";
          Print("[SMC-GOM] 🚫 Pipeline ", action, " ", _Symbol,
                " BLOQUÉ — GOM ", effVerdict, " ", (g_smcGomVerdictNum > 0 ? "BUY" : "SELL"),
                " (vn=", g_smcGomVerdictNum, ") direction opposée");
          return;
       }
    }
