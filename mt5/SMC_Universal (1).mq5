...
input string AI_ServerRender    = "https://kolatradebot.onrender.com";  // URL fallback
...
void ExecuteFVGKillBuy()
{
   ...
   SafeTradeBuy(lot, _Symbol, 0, sl, tp, "FVG_Kill BUY");
   ReleaseOpenLock();
...
void ExecuteFVGKillSell()
{
   ...
   SafeTradeSell(lot, _Symbol, 0, sl, tp, "FVG_Kill SELL");
   ReleaseOpenLock();
   if(trade.ResultRetcode() == TRADE_RETCODE_DONE && UseNotifications)
...
void ExecuteSpikeTrade(string direction)
{
   ...
   if(direction == "BUY")
   {
      ...
      if(SafeTradeBuy(lot, _Symbol, 0.0, sl, tp, g_lastSpikeEntryWasEarly ? "SPIKE CHAIN EARLY BUY" : "SPIKE TRADE BUY"))
      {
         orderExecuted = true;
   ...
   else // SELL
   {
      ...
      if(SafeTradeSell(lot, _Symbol, 0.0, sl, tp, g_lastSpikeEntryWasEarly ? "SPIKE CHAIN EARLY SELL" : "SPIKE TRADE SELL"))
      {
         orderExecuted = true;
