...
struct SpikeModelPaths
{
   string   symbol;           // "PainX 999"
   string   safeName;         // "PainX_999"
   string   amplitudeFile;    // "Models/PainX_999/spike_amplitude.onnx"
   string   intervalFile;     // "Models/PainX_999/spike_interval.onnx"
   string   directionFile;    // "Models/PainX_999/spike_chain_model.onnx" (valid for Vol only)
};

SpikeModelPaths g_spikeModels[];
...
private:
   ...
   bool   LoadOnnx(const string filename, long &handle, const long inputShape[])
   {
...
public:
   ...
   {
      const long inputShape[] = {1, 10};  // 10 features for regression models

      if(ampFile != "")
      {
         if(!LoadOnnx(ampFile, m_ampHandle, inputShape)) return false;
      }
      if(intFile != "")
      {
         if(!LoadOnnx(intFile, m_intHandle, inputShape)) return false;
      }
      if(dirFile != "")
      {
         const long dirShape[] = {1, 8};  // 8 features for direction model
         if(!LoadOnnx(dirFile, m_dirHandle, dirShape)) return false;
      }
   ...
   double PredictAmplitude(const double &features[])
   {
      ...
      float input[10];
      for(int i = 0; i < 10; i++) input[i] = (float)features[i];
      float output[1];
      if(!OnnxRun(m_ampHandle, ONNX_DEFAULT, input, output))
      {
   ...
   double PredictInterval(const double &features[])
   {
      ...
      for(int i = 0; i < 10; i++) input[i] = (float)features[i];
      float output[1];
      if(!OnnxRun(m_intHandle, ONNX_DEFAULT, input, output))
      {
   ...
   double PredictDirectionUp(const double &features[])
   {
      ...
      float input[8];
      for(int i = 0; i < 8; i++) input[i] = (float)features[i];
      float output[2];
      if(!OnnxRun(m_dirHandle, ONNX_DEFAULT, input, output))
      {
