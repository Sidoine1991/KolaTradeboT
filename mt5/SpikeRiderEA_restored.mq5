                         ...
                         const SpikeResult &spike, string &reason)
{
   ...
   if(InpUseTVBridge && InpBlockCounterTrendTV)
   {
      string tvWhy;
      if(!EnsureTVBridgeReady(tvWhy))
      {
         reason = tvWhy;
         return false;
      }
   ...
   if(InpBlockCounterTrendTV && InpUseTVBridge)
   {
      if(g_tvCounterTrend)
      {
         reason = StringFormat("TV VERDICT contre-tendance (EMA=%s OB=%s CT=true)",
...
bool EnsureTVBridgeReady(string &reason)
{
   if(!InpUseTVBridge) return true;

   if(g_lastSpikeTVFetch == 0 || !g_spikeTVOk)
   {
      if(!PollSpikeTVState(true))
      {
         reason = "GOM-Bridge déconnecté (/spike-tv-state indisponible)";
         return false;
      }
   }
   ...
   {
      reason = StringFormat("GOM-Bridge expiré (%ds > %ds)",
                            (int)(TimeCurrent() - g_lastSpikeTVFetch),
                            InpTVBridgeMaxAgeSec);
      return false;
   }
   return true;
}
...
void DrawChartIndicators(const SpikeResult &spike, double imminence)
{
   ...
   if(InpUseTVBridge)
   {
      bool tvLive = g_spikeTVOk && g_lastSpikeTVFetch > 0 &&
                    (TimeCurrent() - g_lastSpikeTVFetch) <= InpTVBridgeMaxAgeSec;
      color tvClr = !tvLive ? clrDimGray :
                    g_tvSniperReady ? clrLimeGreen :
                    (g_tvCounterTrend ? clrTomato : clrGold);
      int ageTv = (g_lastSpikeTVFetch > 0) ? (int)(TimeCurrent() - g_lastSpikeTVFetch) : -1;
      ObjLabel("D_TVBridge",
               StringFormat("GOM %s | TV %s | Sniper %s %.0f%% | imm=%.0f%% | Z=%.2f | OB=%s EMA=%s | %ds",
                            tvLive ? "OK" : "OFF",
                            g_tvDirection,
                            (g_tvSniperReady ? "READY" : "---"),
                            g_tvSniperConfidence, g_tvImminencePct, g_tvSpikeZ,
