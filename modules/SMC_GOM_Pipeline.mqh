...
// Déclaré dans SMC_Universal.mq5 — gate signal-first pour le pipeline
bool DisciplineAllowsPipelineAction(const string action);
// Implémentée dans SMC_Universal.mq5
...
bool SMCGP_WantsGOM()
{
   return ShowGOMDashboard || UseGOMVerdictFilter || UseGOMPipeline;
}
...
void SMCGP_PollGOM()
{
   ...
   // Fallback 2: cache TV — uniquement si source TradingView (évite stale PERFECT BUY)
      ...
      && (SMCGP_JsonBool(body, "ok") || StringFind(body, "\"ok\":true") >= 0))
      ok = true;
   // Fallback 3: /gom-verdict (même TF graphique)
   else if(SMCGP_HttpGet("/gom-verdict?symbol=" + sym + "&chart_tf=" + chartTf + "&source=" + srcParam, body, gomTimeout)
           && (SMCGP_JsonBool(body, "ok") || StringFind(body, "\"ok\":true") >= 0))
      ok = true;
...
bool SMCGP_GOMAllowsDirectionEx(const int dir, const bool requireOBTouch)
{
   ...
   // OTE Filter: priorité aux entrées en zone OTE (Fib 61.8%–78.6% du swing HH/LL)
   if(UseOTE && g_smcOteTop > 0 && g_smcOteBot > 0)
   {
      double curPx = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      bool oteDirectionMatch = (dir == 1 && g_smcOteDir >= 0) || (dir == -1 && g_smcOteDir <= 0);
      if(oteDirectionMatch && !g_smcInOTE)
      {
         Print("[GOM-ALLOW] Rejeté: NOT_IN_OTE_ZONE px=", curPx,
               " ote=[", g_smcOteBot, "-", g_smcOteTop, "]");
         return false;
      }
   }
...
void SMCGP_CleanupChartObjects()
{
   ...
   string prefixes[] = {"TM_KOLA_", "TM_OB_", "TM_BB_", "GOM_PRED_", "SMC_OTE_", "SMC_OB_BULL_", "SMC_OB_BEAR_"};
   for(int p = 0; p < ArraySize(prefixes); p++)
      ObjectsDeleteAll(0, prefixes[p]);
   ObjectDelete(0, "TM_OB_LABEL");
...
void SMCGP_PollAndExecutePipeline()
{
   ...
   if(UseSignalFirstDiscipline && !DisciplineAllowsPipelineAction(action))
   {
